import { ApiRole, Part, Project, Session, UserRole, useNewApiSession, useParts, useProjects } from "../datasets";
...
const searchParts = (query: string, parts: Part[]): Part[] => {
    return (parts ?? []).filter(
        (part) => part.PartName.toLowerCase().includes(query) || part.Project.toLowerCase().includes(query)
    );
};
...
export const Parts = () => {
    ...
    const { selected, onSelect } = useMenuSelection(
        allProjects ?? [],
        pathMatcher,
        permaLink,
        allProjects
            ...
            <Row>
                ...
                {selected ? (
                    <Col className="mx-4">
                        ...
                        <PartsTable downloadSession={downloadSession} projectName={selected.Name} parts={parts ?? []} />
                    </Col>
                ) : (
                    <Col>
                        <p>
                            There are no projects currently accepting submissions, but we are working hard to bring you
                            some!
                            <br />
                            Please check <LinkChannel channel={GuildChannel.NextProjectHints} /> for updates.
                        </p>
                    </Col>
                )}
            </Row>
    ...
    return (
        <div className="d-flex justify-content-center">
            <ButtonGroup vertical={window.visualViewport.width < ButtonGroupBreakPoint}>
                ...
                <DownloadButton fileName={props.project.ReferenceTrack} downloadSession={props.downloadSession}>
...
const PartsTable = (props: { downloadSession: Session | undefined; projectName: string; parts: Part[] }) => {
                    ...
                    <tbody>
                        {wantParts.map((part) => (
                            <tr key={`${part.Project}|${part.PartName}`}>
                                ...
                                <td>
                                    <PartDownloads downloadSession={props.downloadSession} part={part} />
                                </td>
                            </tr>
                        ))}
                    </tbody>
...
const PartDownloads = (props: { downloadSession: Session | undefined; part: Part }) => {
    ...
    if (!isEmpty(props.part.ConductorVideo))
        buttons.push(
            <LinkButton key={props.part.ConductorVideo} to={props.part.ConductorVideo} size={"sm"}>
    ...
    return (
        <ButtonGroup className="justify-content-start" vertical={window.visualViewport.width < ButtonGroupBreakPoint}>
            {buttons}
        </ButtonGroup>
    );
};
...
const LinkButton = (props: { to: string; children: string | (string | JSX.Element)[]; size?: "sm" | "lg" }) => {
    return (
