...
const ProjectTable = (props: {
  ...
  const onClickEdit = (project: MixtapeProject) => {
    window.location.href =
      `/mixtape/NewProjectWorkflow?` +
      new URLSearchParams({ name: project.Name });
  };
      ...
      <tbody>
        {props.projects.map((x) => (
          <tr key={x.Name}>
            ...
            <td>
              <Button onClick={() => onClickEdit(x)}>Edit</Button>
            </td>
          </tr>
        ))}
      </tbody>
...
const SubmitTitleButton = (props: {
  ...
  const name =
    props.current.Name != ""
      ? props.current.Name
      : nameToTitle(CurrentMixtape, props.title);
      ...
      <Card.Body>
        <Form>
          ...
          <ul className="list-inline">
            {hosts.map((host) => (
              <li key={host.nick} className="list-inline-item">
                {host.nick + " "}
                <span
                  className={"text-primary"}
                  style={{ cursor: "pointer" }}
                  onClick={() =>
                    setHosts(hosts.filter((x) => x.user.id != host.user.id))
                  }
                >
                  (remove)
                </span>
...
const SubmitHostsButton = (props: {
  ...
  return (
    <Button disabled={isEmpty(props.hosts)} variant={variant} onClick={onClick}>
      Submit
    </Button>
  );
};
