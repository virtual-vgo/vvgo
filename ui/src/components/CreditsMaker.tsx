...
export const CreditsMaker = () => {
  ...
  return (
    <div>
      ...
      <Row>
        <Col>
          <PastaResult title={"Website Pasta"} content={pasta.websitePasta} />
        </Col>
        <Col>
          <PastaResult title={"Video Pasta"} content={pasta.videoPasta} />
        </Col>
        <Col>
          <PastaResult title={"Youtube Pasta"} content={pasta.youtubePasta} />
        </Col>
      </Row>
    </div>
  );
  ...
  return (
    <div>
      <Row>
        ...
        <Col className="m-2">
          <FormLabel>Read Range {helpLink(links.Help.ReadRange)}</FormLabel>
          <FormControl
            type="text"
      ...
      <Row>
        {errorMessage != "" ? (
          <Col className="text-warning mx-2">
...
const ProjectDropdown = (props: {
  ...
  return (
    <div className="d-flex">
      <Dropdown>
        <Dropdown.Toggle variant="light">{buttonText}</Dropdown.Toggle>

        <Dropdown.Menu>
          {projects.map((p) => (
            <Dropdown.Item key={p.Name} onClick={() => updateSelect(p)}>
              {p.Title}
            </Dropdown.Item>
          ))}
        </Dropdown.Menu>
